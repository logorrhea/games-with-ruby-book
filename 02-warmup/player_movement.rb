...
class Tank
    ...
    def update
        ...
        @body_angle = change_angle(
            @body_angle, Gosu::KbW, Gosu::KbS, Gosu::KbA, Gosu::KbD)
    end
    ...
    def change_angle(previous_angle, up, down, right, left)
        if @window.button_down?(up)
            ...
            angle = 180.0
            angle -= 45 if @window.button_down?(left)
            angle += 45 if @window.button_down?(right)
...
class GameWindow < Gosu::Window
    ...
    UNIT_FILE = File.join(
        File.dirname(File.dirname(__FILE__)), 'media', 'ground_units.json')
    SPEED = 5

    def initialize
        super(640, 480, false)
        @map = Gosu::Tiled.load_json(self, MAP_FILE)
        @units = Gosu::TexturePacker.load_json(self, UNIT_FILE, :precise)
        @tank = Tank.new(self,
